...
def loadimages():
    #loads the sprites for only the characters that are in play
    #also loads other images that are in the game by default
    global loading
...
#DECLARING VARIABLES
#========================================
#when a player performs an attack or is hit by an attack,
#these lists keep track of how long the player cannot hit for
global p1delay,p2delay,WAITING,WAITTIME
p1delay = [False,False]
p2delay = [False,False]
#time waited since last attack/damage taken, amt of loops needed need to wait

WAITING = 0
WAITTIME = 1

#tracks all the hadoukens (fireballs) thrown by each player;
#I have one that never gets deleted and just draws the ball
#off-screen, so that len(hadouken1) never is False
...
#for each entry: [x,y,facing (left or right)]

#These lists represent the frame number, and the movement that each player is doing
#ex. ["walk",4] would mean that the player is doing the walk animation and is on the
#fourth frame of it, and therefore the program will draw this to the screen
...
#These lists represent most of the stats of the two players;
#these lists represent:
# -facing (left or right)
# -if they can attack
# -if they are in an attack
# -what character the player is (ryu,ken,m.bison)
# -if they are on the ground
# -the player's y-velocity (if they are not on the gound)
# -if the player is moving, if not the idle animation will run
# -the player's sprite (the actual image)
# -when the player attack's, how much damage will be dealt
# -the player's health
...
#sets the player's X,Y.Width,Height values and makes them into a hit-box;
#the different sized sprites for m.bison made it so that I had to make an
#if/else statement to make his hit-box
...
#creates rects for the out of bounds areas, so players cannot cross
global outleft,outright,score,menuscreen
outleft = Rect(1024,0,100,1024)
outright = Rect(-100,0,100,1024)

#sets the score list to 1-1 so that the main loop doesn't run initally, and the game goes to a menu screen first
score = [1,1]
#p1 score, p2 score
...
def restart():
    ...
    player2 = [False,True,False,False,True,False,False,False,False,100,True]
    #facing right, can att, is att, character, on ground, y-velocity, moving,
    #points of damage for attack, health,alive


    p1hitbox = [700,670-89,0,0] #x,y,width,height
    p1hit = Rect(p1hitbox)

    p2hitbox = [700,670-89,0,0] #x,y,width,height
    p2hit = Rect(p2hitbox)
